...
function JsImplementationSlide() {
  ...
  const cards: JsCard[] = [
    {
      kicker: "Listen loop",
      caption: "Stream speech, commit only on a real pause.",
      filename: "VoiceInterface.tsx",
      highlight: [5, 6, 7, 8],
      color: "var(--kn-cyan)",
      lines: [
        <>{k("const")} {idt("recognition")} {punc("=")} {k("new")} {fn("SpeechRecognition")}{punc("();")}</>,
        <>{idt("recognition")}{punc(".")}{idt("continuous")} {punc("=")} {k("true")}{punc(";")}</>,
        <>{idt("recognition")}{punc(".")}{idt("interimResults")} {punc("=")} {k("true")}{punc(";")}</>,
        <></>,
        <>{idt("recognition")}{punc(".")}{idt("onresult")} {punc("=")} {punc("(")}{idt("e")}{punc(") => {")}</>,
        <>  {idt("utterance")} {punc("+=")} {fn("consume")}{punc("(")}{idt("e")}{punc(");")}{"  "}{cmt("// final + interim")}</>,
        <>  {fn("clearTimeout")}{punc("(")}{idt("pauseTimer")}{punc(");")}</>,
        <>  {idt("pauseTimer")} {punc("=")} {fn("setTimeout")}{punc("(")}{fn("match")}{punc(", 500);")}</>,
        <>{punc("};")}</>,
      ],
    },
    {
      kicker: "Match phrases",
      caption: "Normalise, then route to deterministic actions.",
      filename: "commands.ts",
      highlight: [4, 5, 6, 7],
      color: "var(--kn-violet)",
      lines: [
        <>{k("function")} {fn("match")}{punc("(")}{idt("text")}{punc(") {")}</>,
        <>  {k("const")} {idt("t")} {punc("=")} {fn("normalize")}{punc("(")}{idt("text")}{punc(");")}{"  "}{cmt("// lowercase, strip punc")}</>,
        <></>,
        <>  {k("if")} {punc("(")}{idt("t")}{punc(".")}{fn("includes")}{punc("(")}{str('"next"')}{punc("))")}         {k("return")} {fn("advance")}{punc("();")}</>,
        <>  {k("if")} {punc("(")}{idt("t")}{punc(".")}{fn("includes")}{punc("(")}{str('"previous"')}{punc("))")}    {k("return")} {fn("retreat")}{punc("();")}</>,
        <>  {k("if")} {punc("(")}{idt("t")}{punc(".")}{fn("includes")}{punc("(")}{str('"pause timer"')}{punc("))")} {k("return")} {fn("pause")}{punc("();")}</>,
        <>  {k("if")} {punc("(")}{idt("t")} {punc("===")} {str('"agenda"')}{punc(")")}              {k("return")} {fn("goTo")}{punc("(")}{str('"agenda"')}{punc(");")}</>,
        <>{punc("}")}</>,
      ],
    },
    {
      kicker: "Dispatch + feedback",
      caption: "Fire named events. UI listens, deck reacts.",
      filename: "ui.tsx",
      highlight: [2, 5, 8],
      color: "var(--kn-magenta)",
      lines: [
        <>{cmt("// safe, named events - the app owns execution")}</>,
        <>{fn("dispatchEvent")}{punc("(")}{k("new")} {fn("CustomEvent")}{punc("(")}{str('"deck:advance"')}{punc("));")}</>,
        <></>,
        <>{cmt("// live caption clears 1.2s after the speaker pauses")}</>,
        <>{fn("dispatchSubtitleUpdate")}{punc("(")}{idt("utterance")} {punc("+")} {idt("interim")}{punc(");")}</>,
        <></>,
        <>{cmt("// flash a 'Matched' pill on the waveform")}</>,
        <>{fn("updateStatus")}{punc("(")}{str('"matched"')}{punc(",")} {punc("{")} {idt("resetAfterMs")}{punc(":")} 1000 {punc("});")}</>,
      ],
    },
  ];
